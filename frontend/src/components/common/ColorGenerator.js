...
export function useColorGenerator() {
  ...
  const methods = {
     ...
     */
    getRandomColor() {
      const allColors = [...COMMON_COLORS.warm, ...COMMON_COLORS.cool, ...COMMON_COLORS.neutral];
    ...
    getRandomByTone(toneType) {
      ...
      return colors[Math.floor(Math.random() * colors.length)] || this.getRandomColor();
    },
